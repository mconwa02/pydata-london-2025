...
from stable_baselines3 import TD3, PPO, SAC, DDPG, DQN
...
class BaseHyperParameterTuning:
    ...
    @staticmethod
    def createModelRootPath(rl_algo_type: RLAgorithmType, problem_title: str) -> str:
        """
        ...
        """
        path = configs.TUNED_MODEL_PATH.format(rl_algo_type.name, problem_title)
    ...
    def createHyperparameterPath(tuned_model_path: str) -> str:
        ...
        """
        tuned_model_parameters_path = f"{tuned_model_path}{configs.TUNED_PARAMETER_FILE_NAME}"
        return tuned_model_parameters_path
        ...
        match self._rl_algo_type:
            case RLAgorithmType.td3 | RLAgorithmType.ddpg:
                ...
                return {
                    ...
                    "train_freq": configs.SB3_CHECK_FREQUENCY,
                }
            ...
            case RLAgorithmType.ppo | RLAgorithmType.dqn:
                return {
                    "policy": "MlpPolicy",
                    "env": self._env,
                }
    ...
    def createModel(self, kwargs: Dict[str, Any]) -> BaseAlgorithm:
        ...
        tensorboard_log_path = f"{configs.HYPER_PARAMETER_TENSORBOARD_FOLDER}/{self._rl_algo_type}"
    ...
    def objective(
        ...
        eval_callback = TrialEvalCallback(
            eval_env,
            trial,
            n_eval_episodes=configs.SB3_N_EVALUATION_EPISODES,
            eval_freq=configs.SB3_EVAL_FREQ,
            deterministic=True
        )
        ...
        try:
            model.learn(
                total_timesteps=configs.SB3_N_TUNING_TRAIN_STEPS,
            ...
            print(f"Training reward: {mean_reward} +/-{std_reward} for {configs.SB3_N_TUNING_TRAIN_STEPS} steps")
            self._all_rewards.append(mean_reward)
    ...
    def run(self) -> str:
        ...
        sampler = TPESampler(n_startup_trials=configs.SB3_N_STARTUP_TRIALS)

        # Do not prune before 1/3 of the max budget is used.
        pruner = MedianPruner(
            n_startup_trials=configs.SB3_N_STARTUP_TRIALS,
            n_warmup_steps=configs.SB3_N_EVALUATIONS)
        ...
        try:
            study.optimize(
                self.objective,
                n_trials=configs.SB3_N_TRIALS,
                timeout=configs.SB3_TUNING_TIMEOUT)
    ...
    def _reportResults(
        ...
        # Write report
        result_path = configs.HYPER_PARAMETER_RESULT_PATH.format(self._rl_algo_type.name)
        optimization_history_path = f"{self._tuned_model_root_path}{configs.HYPER_PARAMETER_HISTORY_PATH}"
        param_importance_path = f"{self._tuned_model_root_path}{configs.HYPER_PARAMETER_IMPORTANCE_PATH}"
    ...
    def plotRewardCurve(
        ...
        :return: None
        """
        reward_curve_path = configs.HYPER_PARAMETER_REWARD_CURVE_PATH.format(self._rl_algo_type.name)
        reward_curve_data_path = configs.HYPER_PARAMETER_REWARD_CURVE_DATA_PATH.format(self._rl_algo_type.name)
